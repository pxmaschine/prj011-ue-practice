...
int32 UUPSignificanceSettings::GetBucketIndex(FName InTag, int32 InIndex) const
{
	...
	for (int BucketIndex = 0; BucketIndex < BucketInfo->BucketSizes.Num(); ++BucketIndex)
	{
		...
		TotalIndex += FMath::Floor(BucketInfo->BucketSizes[BucketIndex] * SignificanceManager::SignificanceBucketSizeMultiplier);
...
FName UUPSignificanceSettings::GetCategoryName() const
{
